...
const appetizersSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchAppetizers.pending, (state) => { state.loading = true; })
      .addCase(fetchAppetizers.fulfilled, (state, action) => { state.items = action.payload; state.loading = false; })
      .addCase(fetchAppetizers.rejected, (state, action) => { state.error = action.payload; state.loading = false; });
  }
});
